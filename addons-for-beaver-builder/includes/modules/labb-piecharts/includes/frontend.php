...
$settings = apply_filters('labb_piecharts_' . esc_attr($id) . '_settings', $settings);
...
$output = '<div class="labb-piecharts labb-grid-container ' . esc_attr(labb_get_grid_classes($settings)) . '">';
...
foreach ($settings->piecharts as $piechart):
    ...
    $child_output .= '<div class="labb-percentage"' . $bar_color . $track_color . ' data-percent="' . esc_attr(round($piechart->percentage)) . '">';

    $child_output .= '<div class="labb-percentage-value">';

    $child_output .= '<span>' . esc_html(round($piechart->percentage)) . '<sup>' . esc_html($settings->percent_symbol) . '</sup></span>';
