...
$settings = apply_filters('labb_team_members_' . esc_attr($id) . '_settings', $settings);
...
$output = '<div class="labb-team-members labb-' . esc_attr($settings->style) . ' ' . esc_attr($container_style) . '">';
...
foreach ($settings->team_members as $team_member):
    ...
    $child_output = '<div class="' . esc_attr($item_style) . ' labb-team-member-wrapper">';

    list($animate_class, $animation_attr) = labb_get_animation_atts($team_member->member_animation);

    $child_output .= '<div class="labb-team-member ' . esc_attr($animate_class) . '" ' . esc_attr($animation_attr) . '>';
    ...
    $child_output .= '<' . esc_html($settings->title_tag) . ' class="labb-title">' . esc_html($team_member->member_name) . '</' . esc_html($settings->title_tag) . '>';

    $child_output .= '<div class="labb-team-member-position">';

    $child_output .= wpautop($wp_embed->autoembed(do_shortcode(wp_kses_post($team_member->member_position))));

    $child_output .= '</div>';

    $child_output .= '<div class="labb-team-member-details">';

    $child_output .= wpautop($wp_embed->autoembed(do_shortcode(wp_kses_post($team_member->member_details))));
